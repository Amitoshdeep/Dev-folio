...
function Navbar() {
  ...
  useEffect(() => {
    setNavAct(loc.pathname === '/');
  }, [loc.pathname]);

  return (
    <AnimatePresence mode="wait">
      <motion.div
        key={navActive ? "active" : "inactive"} // This triggers animation when navActive changes
        className={`p-4 duration-600 text-stone-400 flex
        ${navActive ? "pl-20 flex-col gap-5 justify-center h-full" : "justify-between items-center p-4 px-10 sticky top-0 backdrop-blur-3xl bg-black z-10"}`}
      ...
      >
        ...
        <ul className='flex gap-4 text-lg h-7'>
          ...
          <NavLink to="/About" className={({ isActive }) => `navbar ${isActive ? "active" : ""}`}>
            About
            <div className='underNav'></div>
          </NavLink>
          <NavLink to="/Resume" className={({ isActive }) => `navbar ${isActive ? "active" : ""}`}>
            Resume
            <div className='underNav'></div>
          </NavLink>
          <NavLink to="/Contact" className={({ isActive }) => `navbar ${isActive ? "active" : ""}`}>
            Contact
            <div className='underNav'></div>
          </NavLink>
        </ul>
        ...
        {/* Social Links */}
        <div className={`gap-3.5 cursor-pointer p-1 ${navActive ? "flex" : "hidden"}`}>
